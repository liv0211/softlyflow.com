...
export default function CopyButton({ text, className = '' }: Props) {
  ...
  return (
    <button
      onClick={handleCopy}
      className={`inline-flex items-center gap-1.5 text-xs font-sans font-light tracking-widest uppercase px-3 py-1.5 rounded-sm border transition-all ${
        copied
          ? 'bg-accent border-accent text-cream'
          : 'border-stone-200 text-ink-muted hover:border-accent hover:text-accent'
      } ${className}`}
